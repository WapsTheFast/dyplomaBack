...
struct QuestionsController: RouteCollection {
  func boot(routes: RoutesBuilder) throws {
    let questions = routes.grouped("api", "questions")
  ...
  func getForLecture(req: Request) async throws -> Questions {
    ...
    guard
      let questions = try await Questions.query(on: req.db)
  ...
  func delete(req: Request) async throws -> HTTPStatus {
    guard let questions = try await Questions.find(req.parameters.get("questionsID"), on: req.db)
    else {
      throw Abort(.notFound)
    }
